...
interface FilterPanelProps {
  ...
  languages: string[];
  filters: { language: string; min_stars: number };
  maxStars: number;
  loading: boolean;
  onFilterChange: (filters: { language: string; min_stars: number }) => void;
  onResetFilters: () => void;
  onRandom: () => void;
  error: string | null;
}
...
export default function FilterPanel({
  ...
  const hasActiveFilters = filters.language || filters.min_stars > 0;
